...
const BookOrderSuccess: React.FC<BookOrderSuccessProps> = ({
  ...
  // Retry handler for timed-out ebook generation - actually re-triggers generation
  const handleRetryEbook = async () => {
    if (!orderDetails?.id || isRetrying) return;

    setIsRetrying(true);
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) {
        console.error('[BookOrderSuccess] No session for retry');
        return;
      }

      // Call process-ebook-order to regenerate the PDF
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL || 'https://cdhymstkzhlxcucbzipr.supabase.co'}/functions/v1/process-ebook-order`,
        {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({ bookOrderId: orderDetails.id }),
        }
      );

      if (!response.ok) {
        console.error('[BookOrderSuccess] Retry failed:', await response.text());
      } else {
        console.log('[BookOrderSuccess] Ebook regeneration triggered');
      }
    } catch (err) {
      console.error('[BookOrderSuccess] Error retrying ebook:', err);
    } finally {
      setIsRetrying(false);
              ...
              <>
                ...
                <div className="flex flex-col gap-3">
                  <button
                    ...
                    disabled={isRetrying}
                    className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-pacific-cyan text-white rounded-xl font-bold hover:bg-pacific-cyan/90 transition-colors disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                    {isRetrying ? (
                      <>
